...
| raw `<input type="checkbox" role="switch">` | the Switch primitive's look (36×20 track, `brand-primary` when on, 16px thumb, 120ms slide) driven by the same controlled checkbox through `:checked`. The invisible input fills its box, so the box carries a 4px slop (with a cancelling negative margin) — a 44×28 target around the unchanged track, and the workspace ROW is 28px tall so that whole target stays inside its own row: at a 20px row two neighbours' hit boxes overlapped by 6px and the lower input, painting later, stole the band (the bottom ~2px of a switch toggled the row below). The keyboard ring draws that same box (`outline-offset: 4px`), and the row's label stretches to the full 28px band so no dead strip sits beside a live switch. In a workspace row the label spans the WHOLE text run and NO per-row state word is rendered at all — the switch carries on/off by itself (`role="switch"` + `checked`; a column of identical labels is noise, and the word used to swallow clicks as an inert sibling of the label). The expanded block LEADS with the bulk pair so a long list cannot push it past the fold, and past five workspaces it grows a name filter whose narrowing scopes that pair (`All on (N shown)`, and the pair is inert when nothing matches); the query is dropped when its input goes away, so deleting workspaces down past the threshold cannot strand hidden rows behind a control that is no longer there. That inline filter keeps the field GEOMETRY but not the official field fill: `bg-layer-1` reads as a white slab against the card's `bg-layer-3`, so it takes a transparent fill with the `border-l3` hairline our outline buttons use (the brand focus border stays). The rows are a responsive `auto-fill / minmax(200px, 1fr)` grid (a truncated name keeps its full text in `title`), and the row order is frozen when the list opens (enabled first) so a toggle never moves a row out from under the pointer. Row toggles are QUEUED behind one another rather than blocking: the controller judges a save against the document it read, so overlapping writes would report a spurious conflict — but making every row inert for the duration dimmed all switches on each toggle and read as the card blinking. The queue keeps the writes serial and only the row whose write is running carries `data-pending`/`aria-busy` |
